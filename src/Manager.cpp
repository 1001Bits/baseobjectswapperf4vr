...
namespace FormSwap
{
	...
	void Manager::PrintConflicts() const
	{
		if (const auto console = RE::ConsoleLog::GetSingleton(); hasConflicts) {
			console->PrintLine(std::format("[BOS] Conflicts found, check {}.log in {} for more info\n", Version::PROJECT, logger::log_directory()->string()).c_str());
		}
	}
	...
	SwapFormResult Manager::GetSwapData(RE::TESObjectREFR* a_ref, const RE::TESForm* a_base, const RE::BGSMaterialSwap* a_materialSwap)
	{
		...
		if (const auto swapLvlBase = swapData.first ? swapData.first->As<RE::TESLevItem>() : nullptr) {
			// VR's CommonLibF4 has no TESObjectREFR::GetEncounterZone (NG-only); route via the parent cell.
			const auto parentCell = a_ref->GetSaveParentCell();
			const auto encZone = parentCell ? parentCell->GetEncounterZone() : nullptr;
			if (encZone == nullptr) {
